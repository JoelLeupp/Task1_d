...
class Seminar(Module):
    ...
    def __str__(self):
        return "Seminar in %s under the topic: %s" %(self.title,self.topic)
...
class Thesis(Module):
    ...
    def __str__(self):
        return "Bachelor Thesis on the topic: %s in the reserach group %s" %(self.topic,self.research_group)
